...
def get_results(asin):
	...
	_dict["price"] = soup.find(id="style_name_0_price").text

	#product_date = soup.find(id="averageCustomerReviews").text

	_tech_details = {}
	tech_details_1 = soup.find(id="productDetails_techSpec_section_1").find_all("tr")
	for _tr in tech_details_1:
		_th = _tr.th.get_text()
		_td = _tr.td.get_text()
		_tech_details[_th] = _td

	tech_details_2 = soup.find(id="productDetails_techSpec_section_2").find_all("tr")
	...
	_reviews =  soup.find_all('div', {'class' : "a-expander-content a-expander-partial-collapse-content"})
	"""
	reviews = []
